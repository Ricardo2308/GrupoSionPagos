...
import { getEnviadosBanco } from '../../../../services/getEnviadosBanco'
...
const EnviadosBanco = (prop) => {
  ...
  const OrdenarPorColumna = sessionStorage.getItem('OrdenarPorColumnaEB' + prop.tipo)
  const OrdenarPorDireccion = sessionStorage.getItem('OrdenarPorDireccionEB' + prop.tipo)

  useEffect(() => {
    let mounted = true
    getEnviadosBanco(session.id, prop.tipo, session.api_token).then((items) => {
      if (mounted) {
  ...
  function Ordenamiento(columna, direccion, e) {
    if (columna.name !== undefined) {
      sessionStorage.setItem('OrdenarPorColumnaEB' + prop.tipo, columna.name)
    }
    if (direccion) {
      sessionStorage.setItem('OrdenarPorDireccionEB' + prop.tipo, direccion)
    }
  }
