...
import { getReemplazos } from '../../../../services/getReemplazos'
...
const Reemplazos = (prop) => {
  ...
  const filteredItemsR = reemplazos
  const OrdenarPorColumna = sessionStorage.getItem('OrdenarPorColumnaRS' + prop.tipo)
  const OrdenarPorDireccion = sessionStorage.getItem('OrdenarPorDireccionRS' + prop.tipo)
  ...
  useEffect(() => {
    let mounted = true
    if (location.tipo) {
      setReemplazos(location.reemplazos)
      getReemplazos(session.id, location.tipo, session.api_token).then((items) => {
        if (mounted) {
          ...
          sessionStorage.setItem('listaPagosReemplazos', JSON.stringify(datosOrdenados))
        }
      })
      ...
      getReemplazos(session.id, prop.tipo, session.api_token).then((items) => {
        if (mounted) {
          if (OrdenarPorColumna !== null && OrdenarPorDireccion !== null) {
            if (OrdenarPorColumna == 'Empresa' && OrdenarPorDireccion == 'asc') {
              items.bitacora.sort((a, b) =>
  ...
  function Ordenamiento(columna, direccion, e) {
    if (columna.name !== undefined) {
      sessionStorage.setItem('OrdenarPorColumnaRS' + prop.tipo, columna.name)
    }
    if (direccion) {
      sessionStorage.setItem('OrdenarPorDireccionRS' + prop.tipo, direccion)
    }
