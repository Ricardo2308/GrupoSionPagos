...
import { getAceptadosBanco } from '../../../../services/getAceptadosBanco'
...
const AceptadosBanco = (prop) => {
  ...
  const OrdenarPorColumna = sessionStorage.getItem('OrdenarPorColumnaAB' + prop.tipo)
  const OrdenarPorDireccion = sessionStorage.getItem('OrdenarPorDireccionAB' + prop.tipo)

  useEffect(() => {
    let mounted = true
    getAceptadosBanco(session.id, prop.tipo, session.api_token).then((items) => {
      if (mounted) {
  ...
  function Ordenamiento(columna, direccion, e) {
    if (columna.name !== undefined) {
      sessionStorage.setItem('OrdenarPorColumnaAB' + prop.tipo, columna.name)
    }
    if (direccion) {
      sessionStorage.setItem('OrdenarPorDireccionAB' + prop.tipo, direccion)
    }
  }
