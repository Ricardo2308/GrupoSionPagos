...
import { getConsultorAutorizados } from 'src/services/getConsultorAutorizados'
...
const ConsultorAutorizados = (prop) => {
  ...
  useEffect(() => {
    ...
    getConsultorAutorizados(idUsuario, session.api_token).then((items) => {
      if (mounted) {
        if (colorFiltro === 'NO') {
        ...
        sessionStorage.setItem('ConsultorPagosAutorizados', JSON.stringify(datosOrdenados))
      }
    })
...
export default ConsultorAutorizados
