...
import { getConsultorCancelados } from 'src/services/getConsultorCancelados'
...
const ConsultorCancelados = (prop) => {
  ...
  useEffect(() => {
    ...
    getConsultorCancelados(idUsuario, session.api_token).then((items) => {
      if (mounted) {
        if (colorFiltro === 'NO') {
        ...
        sessionStorage.setItem('ConsultorPagosCancelados', JSON.stringify(datosOrdenados))
      }
    })
...
export default ConsultorCancelados
