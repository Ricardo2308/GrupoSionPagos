...
import { getConsultorPagadosBanco } from 'src/services/getConsultorPagadosBanco'
...
const ConsultorPagadosBanco = (prop) => {
  ...
  useEffect(() => {
    ...
    getConsultorPagadosBanco(idUsuario, session.api_token).then((items) => {
      if (mounted) {
        if (colorFiltro === 'NO') {
        ...
        sessionStorage.setItem('ConsultorPagosPagadosBanco', JSON.stringify(datosOrdenados))
      }
    })
...
export default ConsultorPagadosBanco
