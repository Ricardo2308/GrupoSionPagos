...
import { getConsultorNoVisados } from 'src/services/getConsultorNoVisados'
  ...
  useEffect(() => {
    ...
    getConsultorNoVisados(idUsuario, session.api_token).then((items) => {
      if (mounted) {
        ...
        sessionStorage.setItem('ConsultorPagosNoVisados', JSON.stringify(datosOrdenados))
      }
    })
...
export default ConsultorNoVisados
