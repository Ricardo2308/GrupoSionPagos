...
import { getConsultorCompensados } from 'src/services/getConsultorCompensados'
...
const ConsultorCompensados = (prop) => {
  ...
  useEffect(() => {
    ...
    getConsultorCompensados(idUsuario, session.api_token).then((items) => {
      if (mounted) {
        ...
        sessionStorage.setItem('ConsultorPagosCompensados', JSON.stringify(datosOrdenados))
      }
    })
...
export default ConsultorCompensados
