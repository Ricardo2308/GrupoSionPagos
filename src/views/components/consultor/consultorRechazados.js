...
import { getConsultorRechazados } from 'src/services/getConsultorRechazados'
...
const ConsultorRechazados = (prop) => {
  ...
  useEffect(() => {
    ...
    getConsultorRechazados(idUsuario, session.api_token).then((items) => {
      if (mounted) {
        ...
        sessionStorage.setItem('ConsultorPagosRechazados', JSON.stringify(datosOrdenados))
      }
    })
...
export default ConsultorRechazados
