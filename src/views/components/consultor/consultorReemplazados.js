...
import { getConsultorReemplazados } from 'src/services/getConsultorReemplazados'
...
const ConsultorReemplazados = (prop) => {
  ...
  useEffect(() => {
    ...
    getConsultorReemplazados(idUsuario, session.api_token).then((items) => {
      if (mounted) {
        ...
        sessionStorage.setItem('ConsultorPagosReemplazados', JSON.stringify(datosOrdenados))
      }
    })
...
export default ConsultorReemplazados
