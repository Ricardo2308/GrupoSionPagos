...
import { getConsultorRechazadosBanco } from 'src/services/getConsultorRechazadosBanco'
...
const ConsultorRechazadosBanco = (prop) => {
  ...
  useEffect(() => {
    ...
    getConsultorRechazadosBanco(idUsuario, session.api_token).then((items) => {
      if (mounted) {
        if (colorFiltro === 'NO') {
        ...
        sessionStorage.setItem('ConsultorPagosRechazadosBanco', JSON.stringify(datosOrdenados))
      }
    })
...
export default ConsultorRechazadosBanco
