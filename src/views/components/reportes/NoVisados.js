...
import { getReporteNoVisados } from '../../../services/getReporteNoVisados'
import spanish from '../../../lenguaje/es.json'
import '../../../scss/estilos.scss'
import { CFormSelect } from '@coreui/react'
import { FaSearch } from 'react-icons/fa'

const NoVisados = (prop) => {
  ...
  useEffect(() => {
    ...
    setYears(years)
    let pagos = []
    getReporteNoVisados(session.id, null, null, session.api_token).then((items) => {
      if (mounted) {
        pagos.push(
          { 
  ...
  const filtrar = async () => {
    if(final >= inicio){
      let pagos = []
      getReporteNoVisados(session.id, inicio, final, session.api_token).then((items) => {
        pagos.push(
          { 
            "empresa_nombre" : { type: "string" },
            "doc_num" : { type: "string"},
