...
const PendientesValidacion = (prop) => {
  ...
  useEffect(() => {
    ...
    setYears(years)
    let pagos = []
    getPendientesValidacionReporte(session.id, null, null, form.campo, session.api_token).then((items) => {
      if (mounted) {
        pagos.push(
          { 
  ...
  const filtrar = async () => {
    if(final >= inicio){
      let pagos = []
      getPendientesValidacionReporte(session.id, inicio, final, form.campo, session.api_token).then((items) => {
        pagos.push(
          { 
            "empresa_nombre" : { type: "string" },
            "doc_num" : { type: "string"},
