...
import { getReportePendientesCompensar } from '../../../services/getReportePendientesCompensar'
...
const PendientesCompensar = (prop) => {
  ...
  useEffect(() => {
    ...
    setYears(years)
    let pagos = []
    getReportePendientesCompensar(session.id, null, null, form.campo, session.api_token).then((items) => {
      if (mounted) {
        pagos.push(
          { 
  ...
  const filtrar = async () => {
    if(final >= inicio){
      let pagos = []
      getReportePendientesCompensar(session.id, inicio, final, form.campo, session.api_token).then((items) => {
        pagos.push(
          { 
            "empresa_nombre" : { type: "string"},
            "doc_num" : { type: "string"},
...
export default PendientesCompensar
