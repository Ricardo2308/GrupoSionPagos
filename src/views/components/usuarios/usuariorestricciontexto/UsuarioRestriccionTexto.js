...
import { getUsuarioRestriccionTexto } from '../../../../services/getUsuarioRestriccionTexto'
...
const UsuarioRestriccionTexto = () => {
  ...
  useEffect(() => {
    ...
    getUsuarioRestriccionTexto(null, session.api_token).then((items) => {
      if (mounted) {
  ...
  function mostrarModal(id_usuariorestricciontexto, nombre, opcion) {
    setIdRol(id_usuariorestricciontexto)
    setOpcion(opcion)
    setShow(true)
    setMensaje('Está seguro de eliminar la texto ' + nombre + ' del listado de restricción?')
  }
  ...
  async function eliminarRol(id, opcion) {
    if (opcion == 1) {
      ...
      if (respuesta === 'OK') {
        await getUsuarioRestriccionTexto(null, session.api_token).then((items) => {
          setList(items.restriccion)
        })
      }
    } else if (opcion == 2) {
        ...
        return (
          <div>
            <CButton
              ...
              onClick={() => mostrarModal(row.id_usuariorestricciontexto, row.texto, 1)}
            >
              <FaTrash />
            </CButton>
          </div>
      ...
      <>
        ...
        <div className="float-right" style={{ marginBottom: '10px' }}>
          <CButton
            ...
            size="sm"
            disabled={deshabilitar}
            onClick={() => history.push('/usuariorestricciontexto/nueva')}
          >
            Agregar Nueva
          </CButton>
        </div>
...
export default UsuarioRestriccionTexto
